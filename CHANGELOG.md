...
- Fix the `release` workflow silently publishing a tarball missing the root napi loader (`index.js` / `index.d.ts`), which made `1.0.1` unusable — `require('@effectuate/cubejs-mongosql-driver')` threw `MODULE_NOT_FOUND` ([#7](https://github.com/jmenga/cubejs-mongosql-driver/issues/7)).

  Root cause: those two files are `.gitignore`'d and regenerated by `napi build`, but the `publish` job never runs a rust build (only `build:ts`), so nothing produced them in that job's checkout before `npm pack` ran. `1.0.0` only worked because it was published manually from a local dev environment that happened to have them on disk.

  The `publish` job now stages `index.js` / `index.d.ts` from the host build's artifacts before packing, and a new guard step asserts the packed tarball actually contains them (plus `dist/*`) before anything is published — so a future release-pipeline regression fails CI instead of shipping silently broken.
